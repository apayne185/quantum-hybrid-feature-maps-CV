...
def train_ansatz(X, y, n_qubits=None, layers=2, reps=1, epochs=20, stepsize=0.1, batch_size=32, seed=0):
    ...
    Args:
        ...
        y: array of shape (N,) of binary labels (any two distinct values; the
            larger one is mapped to +1 and the smaller to -1 to match the
            [-1, 1] range of a Pauli-Z expectation value).
    ...
    y_mid = (y.max() + y.min()) / 2
    y_pm1 = np.where(y > y_mid, 1.0, -1.0)
